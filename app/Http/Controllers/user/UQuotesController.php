...
class UQuotesController extends Controller
{
  public function home()
{
    $Quotes = Quotes::select('quotes.subject', 'quotes.id', 'quotes.updated_at', 'quotes.valid_until', 'users.first_name', 'quotes.user_id as quotesuser_id', DB::raw('SUM(quotes_cals.total) as total'), 'users.profile_img', 'users.email', 'quotes.status') // Added 'quotes.status' to the select statement
        ->join('users', 'users.id', 'quotes.customer_name')
        ->join('quotes_cals', 'quotes_cals.quotes_id', 'quotes.id')
        ->groupBy('quotes.id', 'quotes.subject', 'quotes.updated_at', 'quotes.valid_until', 'users.first_name', 'quotes.status') // Added 'quotes.status' to the groupBy statement
        ->where('quotes.customer_name', Auth::user()->id)
        ->orderBy('quotes.created_at', 'desc')
        ->get();
    return view('user.Quotes.home', compact('Quotes'));
}
    ...
    public function view(Request $request,$id)
     {
        ...
        return view('user.Quotes.view',compact('Quotes','QuotesCal','InvoiceSettings','Currency','invoice_id','order_id'));
     }
        ...
        $InvoiceSettings = InvoiceSettings::where('id',1)->first();
        $Currency = Currency::where('is_default',1)->first();
            $pdf = PDF::loadView('user.Quotes.downloadView', ['Currency' => $Currency ,'Quotes' => $Quotes ,'InvoiceSettings' => $InvoiceSettings ,'QuotesCal' => $QuotesCal]);
    ...
    public function printView(Request $request, $id)
    {
        ...
        return view('user.Quotes.printView', compact('Quotes', 'QuotesCal', 'InvoiceSettings','Currency'));
