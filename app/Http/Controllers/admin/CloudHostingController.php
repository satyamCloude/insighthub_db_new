   ...
   public function home(Request $request)
{
    ...
    $orderByServices = Orders::join('users','orders.user_id','users.id')
                      ->join('product_news','orders.product_id','product_news.id')
                      ->join('categories','orders.productCategoryId','categories.id')
                      ->join('producttypes','orders.productTypeId','producttypes.id')
                      ->select('categories.category_name','users.first_name','users.email','users.profile_img','producttypes.name','orders.created_at','orders.order_status','orders.id')
                      ->where('orders.productCategoryId',5)
                      ->get();
                      // echo "<pre>"; print_r($orderByServices); exit;
    return view('admin.Services.CloudHosting.home', compact('CloudHosting','Active','Suspended','Terminated','search','Total','orderByServices'));
}
    ...
    {   
        ...
        $Log['url'] = url('/') . '/admin/CloudHosting/Create';
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);


        return view('admin.Services.CloudHosting.create',compact('Client','Vendor','Product','OperatingSysten','Currency','PaymentMethod','Country','Rack','Status','public_ip',
            'private_ip','Switchs','firewall','Employee')); 
    }
    ...
    public function store(Request $req)
    {
        ...
        $Host['url'] = url('/').'/admin/CloudHosting/store';
        $Host['method'] = "Post";
        ...
        $Log['url'] = url('/') . '/admin/CloudHosting/store';
        $Log['method'] = "Post";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);
        ...
        return redirect('admin/CloudHosting/home')->with('success', "New Cloud Hosting Added Successfully");
    }
    ...
    public function edit(Request $req,$id)
    {
        ...
        $Log['url'] = url('/') . '/admin/CloudHosting/edit/'.$id;
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return view('admin.Services.CloudHosting.edit',compact('CloudHosting','Client','Vendor','Product','OperatingSysten','Currency','PaymentMethod','Country','Rack','Status','public_ip','private_ip','Switchs','firewall','Employee'));
    }
...
public function update(Request $req, $id)
{
    ...
    $data = CloudHosting::find($id);
        ...
        $Host['product_id'] = $req->product_id;
        $Host['type'] = "CloudHosting";
        $Host['url'] = url('/').'/admin/CloudHosting/update/'.$id;
        $Host['method'] = "Post";
        $Host['billing_cycle'] = $req->billing_cycle;
        $Host['singup'] = $req->signup_date;
        $Host['servicestype'] = $req->service_type;
        ...
        $Log['url'] = url('/') . '/admin/CloudHosting/update/'.$id;
        $Log['method'] = "Post";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return redirect('admin/CloudHosting/home')->with('success', "Cloud Hosting Updated Successfully");
    }
     ...
     public function delete(Request $request,$id)
    {
        ...
        $Log['url'] = url('/') . '/admin/CloudHosting/delete/'.$id;
        $Log['method'] = "Post";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);
        return redirect('admin/CloudHosting/home')->with('success', "Cloud Hosting Deleted Successfully");
    }
    ...
    public function EXPORTCSV(Request $request)
        {
            ...
            $Log['url'] = url('/') . '/admin/CloudHosting/EXPORTCSV';
            $Log['method'] = "Get";
            $Log['browser'] = $browser . "-" . $version;
            LogActivity::create($Log);
