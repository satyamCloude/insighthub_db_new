...
class ELicensesController extends Controller
{   
    //home page
    public function home(Request $request)
    {
        ...
        if($RoleAccess[array_search('Licenses', array_column($RoleAccess, 'per_name'))]['view'] == 1)
        {
        ...
        if($RoleAccess[array_search('Licenses', array_column($RoleAccess, 'per_name'))]['view'] == 2)
        {
            $query = $request->get('search');
            ...
            ->where(function($q) use ($query) {
                ...
                ->where('licenses.user_id',Auth::user()->id)
               ->where('total_services.category_id',16)
            ->groupBy('total_services.unique_id')
            ->orderBy('licenses.created_at', 'desc')
            ->paginate(10);
            ...
            $Total = Licenses::where('user_id',Auth::user()->id)->count();
            $Active = Licenses::where('user_id',Auth::user()->id)->where('status', 1)->count();
            $Suspended = Licenses::where('user_id',Auth::user()->id)->where('status', 2)->count();
            $Terminated = Licenses::where('user_id',Auth::user()->id)->where('status', 3)->count();
                    
        }

        return view('Employee.Services.Licenses.home', compact('RoleAccess','Licenses','Active','Suspended','Terminated','query','Total'));
    }
    ...
    {   
        ...
        $Log['url'] = url('/') . '/Employee/Licenses/Create';
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);


        return view('Employee.Services.Licenses.create',compact('Client','Vendor','Product','OperatingSysten','Currency','PaymentMethod','Status','Employee')); 
    }
    ...
    public function store(Request $req)
    {
        ...
        $Host['type'] = "Licenses";
        $Host['url'] = url('/').'/Employee/Licenses/store';
        $Host['method'] = "Post";
        ...
        $Log['url'] = url('/') . '/Employee/Licenses/store';
        $Log['method'] = "Post";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        
        return redirect('Employee/Licenses/home')->with('success', "New Licenses Added Successfully");
    }
    ...
    public function edit(Request $req,$id)
    {
        ...
        $Log['url'] = url('/') . '/Employee/Licenses/edit/'.$id;
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return view('Employee.Services.Licenses.edit',compact('Licenses','Client','Vendor','Product','OperatingSysten','Currency','PaymentMethod','Status','Employee'));
    }
...
public function update(Request $req, $id)
{
        ...
        $Host['product_id'] = $req->product_id;
        $Host['type'] = "Licenses";
        $Host['url'] = url('/').'/Employee/Licenses/update/'.$id;
        $Host['method'] = "Post";
        $Host['billing_cycle'] = $req->billing_cycle;
        $Host['singup'] = $req->signup_date;
        $Host['servicestype'] = $req->service_type;
        ...
        $Log['url'] = url('/') . '/Employee/Licenses/update/'.$id;
        $Log['method'] = "Post";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log); 


        return redirect('Employee/Licenses/home')->with('success', "Licenses Edit Successfully");
    }
     ...
     public function delete(Request $request,$id)
    {
        ...
        $Log['url'] = url('/') . '/Employee/Licenses/delete/'.$id;
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return redirect('Employee/Licenses/home')->with('success', "Licenses Deleted Successfully");
    }
    ...
    public function EXPORTCSV(Request $request)
        {
            ...
            $Log['url'] = url('/') . '/Employee/Licenses/EXPORTCSV';
            $Log['method'] = "Get";
            $Log['browser'] = $browser . "-" . $version;
            LogActivity::create($Log);
