...
{   
    ...
    public function home(Request $request)
    {
        ...
        if($RoleAccess[array_search('Other', array_column($RoleAccess, 'per_name'))]['view'] == 1)
        {
            $query = $request->get('search');
        
             $Other = Other::select('others.service_type','total_services.unique_id as unique_service_id','users.profile_img','users.email','others.id','others.signup_date','others.status','users.first_name','product_news.product_name')
        ...
        if($RoleAccess[array_search('Other', array_column($RoleAccess, 'per_name'))]['view'] == 2)
        {
            $query = $request->get('search');
        $Other = Other::select('others.service_type','total_services.unique_id as unique_service_id','users.profile_img','users.email','others.id','others.signup_date','others.status','users.first_name','product_news.product_name')
            ->join('users','users.id','others.customer_id')
            ...
            ->where(function($q) use ($query) {
                ...
                ->where('others.user_id',Auth::user()->id)
              ->where('total_services.category_id',12)
            ->groupBy('total_services.unique_id')
            ->orderBy('one_time_setups.created_at', 'desc')
                ->paginate(10);
            ...
            $Total = Other::where('user_id',Auth::user()->id)->count();
            $Active = Other::where('user_id',Auth::user()->id)->where('status', 1)->count();
            $Suspended = Other::where('user_id',Auth::user()->id)->where('status', 2)->count();
            $Terminated = Other::where('user_id',Auth::user()->id)->where('status', 3)->count();
                    
        }
        
        return view('Employee.Services.Other.home', compact('RoleAccess','Other','Active','Suspended','Terminated','query','Total'));
    }
    ...
    {   
        ...
        $Log['url'] = url('/') . '/Employee/Other/Create';
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return view('Employee.Services.Other.create',compact('Client','Vendor','Product','OperatingSysten','Currency','PaymentMethod','Status','Employee')); 
    }
    ...
    public function store(Request $req)
    {
        ...
        $Host['type'] = "Other";
        $Host['url'] = url('/').'/Employee/Other/store';
        $Host['method'] = "Post";
        ...
        $Log['url'] = url('/') . '/Employee/Other/store';
        $Log['method'] = "Post";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);
        
        return redirect('Employee/Other/home')->with('success', "New Other Added Successfully");
    }
    ...
    public function edit(Request $req,$id)
    {
        ...
        $Log['url'] = url('/') . '/Employee/Other/edit/'.$id;
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return view('Employee.Services.Other.edit',compact('Other','Client','Vendor','Product','OperatingSysten','Currency','PaymentMethod','Status','Employee'));
    }
...
public function update(Request $req, $id)
{
        ...
        $Host['product_id'] = $req->product_id;
        $Host['type'] = "Other";
        $Host['url'] = url('/').'/Employee/Other/update/'.$id;
        $Host['method'] = "Post";
        $Host['billing_cycle'] = $req->billing_cycle;
        $Host['singup'] = $req->signup_date;
        $Host['servicestype'] = $req->service_type;
        ...
        $Log['url'] = url('/') . '/Employee/Other/update/'.$id;
        $Log['method'] = "Post";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return redirect('Employee/Other/home')->with('success', "Other Edit Successfully");
    }
     ...
     public function delete(Request $request,$id)
    {
        ...
        $Log['url'] = url('/') . '/Employee/Other/delete/'.$id;
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return redirect('Employee/Other/home')->with('success', "Other Deleted Successfully");
    }
    ...
    public function EXPORTCSV(Request $request)
        {
            ...
            $Log['url'] = url('/') . '/Employee/Other/EXPORTCSV';
            $Log['method'] = "Get";
            $Log['browser'] = $browser . "-" . $version;
            LogActivity::create($Log);
