...
class ECompanyLoginController extends Controller
{   
    //home page
    public function home(Request $request)
    {
        ...
        if($RoleAccess[array_search('CompanyLogin', array_column($RoleAccess, 'per_name'))]['view'] == 1)
        {
            $query = $request->get('search');
        
            $CompanyLogin = CompanyLogin::select('company_logins.company_name','company_logins.id','company_logins.portal_login_url','company_logins.username','company_logins.password2','company_logins.status','users.first_name')
         ...
         if($RoleAccess[array_search('CompanyLogin', array_column($RoleAccess, 'per_name'))]['view'] == 2)
        {
            $query = $request->get('search');
            ...
            $CompanyLogin = CompanyLogin::select('company_logins.company_name','company_logins.id','company_logins.portal_login_url','company_logins.username','company_logins.password2','company_logins.status','users.first_name')
                ...
                ->orderBy('company_logins.created_at', 'desc')
                ->where('company_logins.user_id',Auth::user()->id)
                ->paginate(10);

             $CompanyLogin->appends(['search' => $query]);   
        
            $Total = CompanyLogin::where('user_id',Auth::user()->id)->count();
            $Active = CompanyLogin::where('user_id',Auth::user()->id)->where('status', 1)->count();
            $Suspended = CompanyLogin::where('user_id',Auth::user()->id)->where('status', 2)->count();
            $Terminated = CompanyLogin::where('user_id',Auth::user()->id)->where('status', 3)->count();
                    
        }
        
        return view('Employee.CompanyLogin.home', compact('RoleAccess','CompanyLogin','Active','Suspended','Terminated','query','Total'));
    }
    ...
    {   
        ...
        $Log['url'] = url('/') . '/Employee/CompanyLogin/Create';
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return view('Employee.CompanyLogin.create',compact('Employee','Status')); 
    }
...
public function store(Request $req)
{
    ...
    // Log activity
        ...
        $Log['url'] = url('/') . '/Employee/CompanyLogin/store';
        $Log['method'] = "Post";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return redirect('Employee/CompanyLogin/home')->with('success', "New Company Added Successfully");
    }
...
// Upload method
   ...
   public function Upload($StorageSetting, $fileName, $file)
    ...
    public function edit(Request $req,$id)
    {
        ...
        $Log['url'] = url('/') . '/Employee/CompanyLogin/edit/'.$id;
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);
        ...
        $Employee = User::select('first_name','id')->where('type',4)->get();
        return view('Employee.CompanyLogin.edit',compact('CompanyLogin','Employee','Status'));
    }
   ...
   public function update(Request $req, $id)
{
    ...
    // Log activity
        ...
        $Log['url'] = url('/') . '/Employee/CompanyLogin/update/'.$id;
        $Log['method'] = "Post";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);  

        return redirect('Employee/CompanyLogin/home')->with('success', "Company Edit Successfully");
     ...
     public function delete(Request $request,$id)
    {
        ...
        $Log['url'] = url('/') . '/Employee/CompanyLogin/delete/'.$id;
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        CompanyLogin::find($id)->delete();
        return redirect('Employee/CompanyLogin/home')->with('success', "Company Deleted Successfully");
    }
    ...
    public function EXPORTCSV(Request $request)
        {
            ...
            $Log['url'] = url('/') . '/Employee/CompanyLogin/EXPORTCSV';
            $Log['method'] = "Get";
            $Log['browser'] = $browser . "-" . $version;
            LogActivity::create($Log);
