...
class EOneTimeSetupController extends Controller
{   
    //home page
    public function home(Request $request)
    {
        ...
        if($RoleAccess[array_search('OneTimeSetup', array_column($RoleAccess, 'per_name'))]['view'] == 1)
        {
        ...
        if($RoleAccess[array_search('OneTimeSetup', array_column($RoleAccess, 'per_name'))]['view'] == 2)
        {
            $query = $request->get('search');
            ...
            ->where(function($q) use ($query) {
                ...
                ->where('one_time_setups.user_id',Auth::user()->id)
                  ->where('total_services.category_id',12)
            ->groupBy('total_services.unique_id')
            ->orderBy('one_time_setups.created_at', 'desc')
                ->paginate(10);
            $OneTimeSetup->appends(['search' => $query]);
            $Total = OneTimeSetup::where('user_id',Auth::user()->id)->count();
            $Active = OneTimeSetup::where('user_id',Auth::user()->id)->where('status', 1)->count();
            $Suspended = OneTimeSetup::where('user_id',Auth::user()->id)->where('status', 2)->count();
            $Terminated = OneTimeSetup::where('user_id',Auth::user()->id)->where('status', 3)->count();
        ...
        return view('Employee.Services.OneTimeSetup.home', compact('RoleAccess','OneTimeSetup','Active','Suspended','Terminated','query','Total'));
    }
    ...
    {   
        ...
        $Log['url'] = url('/') . '/Employee/OneTimeSetup/Create';
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return view('Employee.Services.OneTimeSetup.create',compact('Client','Vendor','Product','OperatingSysten','Currency','PaymentMethod','Status','Employee')); 
    }
    ...
    public function store(Request $req)
    {
        ...
        $Host['url'] = url('/').'/Employee/OneTimeSetup/store';
        $Host['method'] = "Post";
        ...
        $Log['url'] = url('/') . '/Employee/OneTimeSetup/store';
        $Log['method'] = "Post";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return redirect('Employee/OneTimeSetup/home')->with('success', "New OneTimeSetup Added Successfully");
    }
    ...
    public function edit(Request $req,$id)
    {
        ...
        $Log['url'] = url('/') . '/Employee/OneTimeSetup/edit/'.$id;
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return view('Employee.Services.OneTimeSetup.edit',compact('OneTimeSetup','Client','Vendor','Product','OperatingSysten','Currency','PaymentMethod','Status','Employee'));
    }
...
public function update(Request $req, $id)
{
    ...
    $data = OneTimeSetup::find($id);
        ...
        $Host['product_id'] = $req->product_id;
        $Host['type'] = "OneTimeSetup";
        $Host['url'] = url('/').'/Employee/OneTimeSetup/update/'.$id;
        $Host['method'] = "Post";
        $Host['billing_cycle'] = $req->billing_cycle;
        $Host['singup'] = $req->signup_date;
        $Host['servicestype'] = $req->service_type;
        ...
        $Log['url'] = url('/') . '/Employee/OneTimeSetup/update/'.$id;
        $Log['method'] = "Post";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);


        return redirect('Employee/OneTimeSetup/home')->with('success', "OneTimeSetup Edit Successfully");
    }
     ...
     public function delete(Request $request,$id)
    {
        ...
        $Log['url'] = url('/') . '/Employee/OneTimeSetup/delete/'.$id;
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return redirect('Employee/OneTimeSetup/home')->with('success', "OneTimeSetup Deleted Successfully");
    }
    ...
    public function EXPORTCSV(Request $request)
        {
            ...
            $Log['url'] = url('/') . '/Employee/OneTimeSetup/EXPORTCSV';
            $Log['method'] = "Get";
            $Log['browser'] = $browser . "-" . $version;
            LogActivity::create($Log);
