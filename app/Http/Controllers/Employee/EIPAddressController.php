...
class EIPAddressController extends Controller
{   
    //home page
     ...
     if($RoleAccess[array_search('IPAddress', array_column($RoleAccess, 'per_name'))]['view'] == 1)
     {  
        $query = IPAddress::select('users.first_name', 'i_p_addresses.id', 'i_p_addresses.hostname_id', 'i_p_addresses.ip_address', 'i_p_addresses.status')
     ...
     if($RoleAccess[array_search('IPAddress', array_column($RoleAccess, 'per_name'))]['view'] == 2)
     {  
        $query = IPAddress::select('users.first_name', 'i_p_addresses.id', 'i_p_addresses.hostname_id', 'i_p_addresses.ip_address', 'i_p_addresses.status')
                ->join('users', 'users.id', '=', 'i_p_addresses.customer_id')
                ->Where('i_p_addresses.user_id',Auth::user()->id)
                ->orderBy('i_p_addresses.created_at', 'desc');
            ...
            $Total = IPAddress::Where('user_id',Auth::user()->id)->count();
            $publicIp = IPAddress::Where('user_id',Auth::user()->id)->where('ip_type', '1')->count();
            $privateIp = IPAddress::Where('user_id',Auth::user()->id)->where('ip_type', '2')->count();

     }

            return view('Employee.NetworkManagement.IPAddress.home', compact('RoleAccess','IPAddress', 'publicIp', 'privateIp', 'searchTerm', 'Total'));
        }
    ...
    {   
        ...
        $Log['url'] = url('/') . '/Employee/IPAddress/Create';
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return view('Employee.NetworkManagement.IPAddress.create',compact('Countrys','customer')); 
    }
    ...
    public function store(Request $req)
    {
        ...
        $Log['url'] = url('/') . '/Employee/IPAddress/store';
        $Log['method'] = "Post";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return redirect('Employee/IPAddress/home')->with('success', "New IPAddress Added Successfully");
    }
    ...
    public function edit(Request $req,$id)
    {
        ...
        $Log['url'] = url('/') . '/Employee/IPAddress/edit/'.$id;
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return view('Employee.NetworkManagement.IPAddress.edit',compact('IPAddress','Countrys','customer'));
    }
    ...
    public function update(Request $req,$id)
    {
        ...
        $Log['url'] = url('/') . '/Employee/IPAddress/update/'.$id;
        $Log['method'] = "Post";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return redirect('Employee/IPAddress/home')->with('success', "IPAddress Edited Successfully");
    }
    // delete
     public function delete(Request $request,$id)
    {
        ...
        $Log['url'] = url('/') . '/Employee/IPAddress/delete/'.$id;
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        IPAddress::find($id)->delete();
        return redirect('Employee/IPAddress/home')->with('success', "IPAddress Deleted Successfully");
    }
    ...
    public function ExportCSV(Request $request)
        {
            ...
            $Log['url'] = url('/') . '/Employee/IPAddress/EXPORTCSV';
            $Log['method'] = "Get";
            $Log['browser'] = $browser . "-" . $version;
            LogActivity::create($Log);
