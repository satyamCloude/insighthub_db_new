...
class EMonthelySetupController extends Controller
{   
    //home page
 public function home(Request $request)
{
     $RoleAccess = \App\Models\RoleAccess::select('role_accesses.add','role_accesses.view','role_accesses.update','role_accesses.delete','permissions.name as per_name')
        ...
        if($RoleAccess[array_search('MonthelySetup', array_column($RoleAccess, 'per_name'))]['view'] == 1)
        {
            $query = $request->get('search');

            $MonthelySetup = MonthelySetup::select('monthely_setups.service_type', 'monthely_setups.id', 'monthely_setups.signup_date', 'monthely_setups.status', 'users.first_name', 'products.product_name')
        ...
        if($RoleAccess[array_search('MonthelySetup', array_column($RoleAccess, 'per_name'))]['view'] == 2)
        {
            $query = $request->get('search');
            ...
            $MonthelySetup = MonthelySetup::select('monthely_setups.service_type', 'monthely_setups.id', 'monthely_setups.signup_date', 'monthely_setups.status', 'users.first_name', 'products.product_name')
                ...
                ->orderBy('monthely_setups.created_at', 'desc')
                ->where('monthely_setups.user_id',Auth::user()->id)
                ->paginate(10);

            $MonthelySetup->appends(['search' => $query]); // Append the search term to the pagination links

            $Total = MonthelySetup::where('user_id',Auth::user()->id)->count();
            $Active = MonthelySetup::where('user_id',Auth::user()->id)->where('status', 1)->count();
            $Suspended = MonthelySetup::where('user_id',Auth::user()->id)->where('status', 2)->count();
            $Terminated = MonthelySetup::where('user_id',Auth::user()->id)->where('status', 3)->count();
                    
        }

    return view('Employee.Services.MonthelySetup.home', compact('RoleAccess','MonthelySetup', 'Total', 'Active', 'Suspended', 'Terminated', 'query'));
}
    ...
    {   
        ...
        $Log['url'] = url('/') . '/Employee/MonthelySetup/Create';
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return view('Employee.Services.MonthelySetup.create',compact('Client','Vendor','Product','OperatingSysten','Currency','PaymentMethod','Status','Employee')); 
    }
    ...
    public function store(Request $req)
    {
        ...
        $Host['url'] = url('/').'/Employee/MonthelySetup/store';
        $Host['method'] = "Post";
        ...
        $Log['url'] = url('/') . '/Employee/MonthelySetup/store';
        $Log['method'] = "Post";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return redirect('Employee/MonthelySetup/home')->with('success', "New MonthelySetup Added Successfully");
    }
    ...
    public function edit(Request $req,$id)
    {
        ...
        $Log['url'] = url('/') . '/Employee/MonthelySetup/edit/'.$id;
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return view('Employee.Services.MonthelySetup.edit',compact('MonthelySetup','Client','Vendor','Product','OperatingSysten','Currency','PaymentMethod','Status','Employee'));
    }
...
public function update(Request $req, $id)
{
    ...
    $data = MonthelySetup::find($id);
        ...
        $data['MonthelySetup_notes'] = $req->MonthelySetup_notes;
        ...
        $Host['product_id'] = $req->product_id;
        $Host['type'] = "MonthelySetup";
        $Host['url'] = url('/').'/Employee/MonthelySetup/update/'.$id;
        $Host['method'] = "Post";
        $Host['billing_cycle'] = $req->billing_cycle;
        $Host['singup'] = $req->signup_date;
        $Host['servicestype'] = $req->service_type;
        ...
        $Log['url'] = url('/') . '/Employee/MonthelySetup/update/'.$id;
        $Log['method'] = "Post";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);


        return redirect('Employee/MonthelySetup/home')->with('success', "MonthelySetup Edit Successfully");
    }
     ...
     public function delete(Request $request,$id)
    {
        ...
        $Log['url'] = url('/') . '/Employee/MonthelySetup/delete/'.$id;
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return redirect('Employee/MonthelySetup/home')->with('success', "MonthelySetup Deleted Successfully");
    }
    ...
    public function EXPORTCSV(Request $request)
        {
            ...
            $Log['url'] = url('/') . '/Employee/MonthelySetup/EXPORTCSV';
            $Log['method'] = "Get";
            $Log['browser'] = $browser . "-" . $version;
            LogActivity::create($Log);
