...
public function home(Request $request)
{
        ...
        if($RoleAccess[array_search('Task', array_column($RoleAccess, 'per_name'))]['view'] == 2)
        {
            $query = Task::select('users.profile_img as project_manager_picture', 'users.first_name as project_manager_name', 'tasks.deadline', 'tasks.status_pro', 'tasks.team_id', 'tasks.id', 'tasks.deadline', 'tasks.status_id','tasks.startDate','tasks.endDate','tasks.completed_on', 'tasks.project_id', 'tasks.task_name') // Fixed: Added 'tasks.task_name'
        ->join('users', 'users.id', '=', 'tasks.user_id')
        ...
        ->orderBy('tasks.created_at', 'desc');
            ...
            $InProgress = Task::where('user_id',Auth::user()->id)->where('status_id', 1)->count();
            $Completed = Task::where('user_id',Auth::user()->id)->where('status_id', 2)->count();
            $OverDue = Task::where('user_id',Auth::user()->id)->where('status_id', 3)->count();
            $TaskCategory = TaskCategory::where('user_id', Auth::user()->id)->get();
            $Cancel = Task::where('user_id', Auth::user()->id)->where('status_id', 4)->count();
                    
        }

    return view('Employee.Task.home', compact('RoleAccess','Task', 'InProgress', 'Completed', 'OverDue', 'Cancel', 'search', 'TaskCategory','Project'));
}
    ...
    {   
        ...
        return view('Employee.Task.create',compact('Employee','Project','TaskCategory')); 
    }
    ...
    public function store(Request $req)
    {
        ...
        $Log['url'] = url('/') . '/Employee/Task/store';
        $Log['method'] = "Post";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);
        
        return redirect('Employee/Task/home')->with('success', "New Task Added Successfully");
    }
    ...
    public function edit(Request $req,$id)
    {
        ...
        return view('Employee.Task.edit',compact('Task','Vendor','Employee','Client','Project','TaskCategory'));
    } 
    //edit
   ...
   public function update(Request $req, $id)
{
    ...
    if ($task) {
        ...
        $Log['url'] = url('/') . '/Employee/Task/update/' . $id;
        $Log['method'] = "Post";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        return redirect('Employee/Task/home')->with('success', "Task Updated Successfully");
    }

    return redirect('Employee/Task/home')->with('error', "Task not found");
}
     ...
     public function delete(Request $request,$id)
    {
        ...
        $Log['url'] = url('/') . '/Employee/Task/delete/'.$id;
        $Log['method'] = "Get";
        $Log['browser'] = $browser . "-" . $version;
        LogActivity::create($Log);

        Task::find($id)->delete();
        return redirect('Employee/Task/home')->with('success', "Task Deleted Successfully");
    }
