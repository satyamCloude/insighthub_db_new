    ...
    $(document).ready(function(){
        ...
        $('#welcometext').on('selectstart', function(e) {
            e.preventDefault();
...
</script>             <div id="passwordError" style="color: red;"></div>
              ...
              @endif
              <form id="formAuthentication" class="mb-3 my-3" action="{{url('user-forgot-password-store')}}" method="POST">
              @csrf
              <div class="mb-3">
                <label for="email" class="form-label">New Password</label>
                <input type="text" class="form-control" id="newPassword" name="newPassword" placeholder="******" autofocus />

                <input type="hidden" id="user_id" name="user_id"  value="{{request()->get('id')}}" />
              </div>
              <div class="mb-3">
              ...
              <button type="button" id="submitForm" class="btn btn-primary d-grid w-100">Submit</button>
            </form>
            <p class="text-center">
...
$(document).ready(function() {
    $('#submitForm').click(function(event) {
       ...
       if(!password.match(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]+$/)) {
                ...
                $('#passwordError').text('* Confirm password should match');
                return;
            } else {
            $('#passwordError').text('');
