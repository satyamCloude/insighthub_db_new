@foreach($users as $key => $user)
    @php  
        $generated_by = \App\Models\User::leftJoin('employee_details', 'employee_details.user_id', 'users.id')
            ...
            ->select('users.first_name', 'users.id', 'users.profile_img')
            ->where('users.id', $user->generated_by)
            ->first();   

        $lead_status = \App\Models\LeadStatus::select('lead_status', 'id', 'label_color')
            ->where('id', $user->leadStatus)
            ->first();           
    @endphp
    <tr class="odd">
        <td>{{ $key + 1 }}</td>
        ...
        <td>
            @foreach($Employee as $Emp)
                @if($user && $user->assignedto == $Emp->id)
                    <img class="rounded-circle" style="margin-right: 15px;margin-top: 10px;" src="{{ isset($Emp->profile_img) ? $Emp->profile_img : url('public/images/21104.png') }}" height="30" width="30" alt="User avatar" />
                    {{ $Emp->first_name }} {{ $Emp->last_name }} (#{{ $Emp->id }})
                    <div style="font-size:12px;margin-left: 46px;margin-top: -11px;">{{ $Emp->desgname }}</div>
                @endif
            @endforeach
        </td>
        <td>
            @if($generated_by && $generated_by->first_name)
                <img class="rounded-circle" style="margin-right: 15px;" src="{{ $generated_by->profile_img }}" height="30" width="30" alt="User avatar" />
                {{ $generated_by->first_name }} {{ $generated_by->last_name }} (#{{ $generated_by->id }})
                <div style="font-size:12px;margin-left: 46px;margin-top: -11px;">{{ $generated_by->desgname }}</div>
            @endif
        </td> 
        <td>
        ...
        <td>
            <div class="d-flex">
                <a href="{{ url('Employee/Leads/view/'.$user->id) }}">
                    <svg style="margin-left: 10px;" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" font-size="1.375rem" width="1em" height="1em" viewBox="0 0 24 24">
                ...
                <a href="{{ url('Employee/Quotes/add/?id='.$user->id) }}">
                    <svg style="margin-left: 10px; fill: #7367f0;" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24">
