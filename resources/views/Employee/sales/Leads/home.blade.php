...
<!-- Users List Table -->
                ...
                <section class="emp_table">
                   ...
                   <button onclick="FollowUps(); "class="btn btn-primary mt-3 m-3 leadbtn">Followup's</button>
                    @if($RoleAccess[array_search('Leads', array_column($RoleAccess, 'per_name'))]['add'] == 1)
                         <a href="{{url('Employee/Leads/add')}}" class="btn btn-primary waves-effect waves-light"> Add</a>
                    @endif
                              <a href="{{ url('Employee/Leads/home') }}" class="btn btn-warning mt-3 m-3"><i class="fas fa-sync-alt"></i></a>
                              <!--<a href="{{url('Employee/recent_follow_ups')}}" class="btn btn-primary mt-3 m-3">All Follow UPs</a>-->
                        </div>
                    </div>
    ...
    <div class="card-datatable table-responsive">
        ...
        <!--        <form id="downloadPDF" action="{{url('Employee/Invoices/downloadPDF')}}">-->
        ...
        <!--            <a href="{{url('Employee/Invoices/EXPORTCSV')}}" class="btn btn-info mt-3 m-3"><i class="fa-solid fa-file-csv"></i></a>-->
        <!--            <a href="{{url('Employee/Invoices/home')}}" class="btn btn-warning mt-3 m-3"><i class="fas fa-sync-alt"></i></a>-->
        <!--            <a href="{{url('Employee/Invoices/gst')}}" class="btn btn-primary mt-3 m-3">GST</a>-->
        <!--            <a href="{{url('Employee/Invoices/tds')}}" class="btn btn-primary mt-3 m-3">TDS</a>-->
        <!--            <a href="{{url('Employee/Invoices/add')}}" class="btn btn-primary mt-3 m-3">Add</a>-->
        <!--        </form>-->
        <!--    </div>-->
        <!--</div>-->
                            ...
                            <div class="parent d-flex">
                                   <div class="child1">
                                       <img 
                                        ...
                                        src="{{ isset($Emp->profile_img) ? $Emp->profile_img : url('public/images/21104.png') }}" 
                                        height="30" 
                                       width="30" 
                                        alt="User avatar" 
                                    />
                    ...
                    <td>
                        <div style="display:flex;align-items:center;">
                            ...
                            <span style="padding: 5px;border-radius: 4px;">
                                @if($leadStatus)
                                    {{ ucfirst($leadStatus->lead_status) }}
                                @else
...
@endif
                ...
                <a href="{{ url('Employee/Leads/delete/'.$user->id) }}" id="{{ $user->id }}" class="delete_debtcase">
                    <svg style="margin-left: 10px" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img" font-size="1.375rem" class="iconify iconify--tabler" width="1em" height="1em" viewBox="0 0 24 24">
                       ...
                       @endif
        @if($empDetl && $empDetl->department_id != 1)
...
<script>
    ...
    $(document).ready(function () {
        ...
        function fetchData() {
            ...
            var selectedMonth = $selectMonth.val();
                  $.ajax({
                url: "{{ url('Employee/Leads/getleadsYearfilterdata') }}",
                method: 'GET',
    ...
    function Leads() {
        $.ajax({
            url: "{{ url('Employee/Leads/ShowLeads') }}",
            method: 'GET',
            success: function(data) {
                // Handle the successful response
    ...
    function FollowUps() {
        $.ajax({
            url: "{{ url('Employee/Leads/ShowFollowUps') }}",
            method: 'GET',
            success: function(data) {
                // Handle the successful response
                $('#newpayroll').empty(); // Clear previous content
