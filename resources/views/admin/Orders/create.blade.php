            ...
            <!--</div>-->
                    <!--<div class="col-2">-->
                      <div class="action-btns">
                                  <a href="{{url('admin/Orders/home')}}" class="btn btn-label-primary me-3 mt-3">
              ...
              <div class="card-body">
                <div class="row mb-4 mt-3"> 
                    <div class="col-md-6">
                        ...
                        <div class="dropdown w-100">
                              <button class="dropbtn" style="justify-content:space-between;margin-right:3%">
                                  <div >
                                <img src="{{url('/')}}/public/images/profile_Ri1o.jpeg" alt="" style="display:none;" class="rounded-circle avatar-xs" id="selected_client_img">
                    ...
                    <div class="col-md-6">
                        ...
                        <!--  <option value="{{ $product->id }}" tax-rate="{{ $product->rate }}" tax-id="{{ $product->tax_id }}" tax-name="{{ $product->tax_name }}">{{ $product->product_name }}</option>-->
                        <!--  @endforeach-->
                        <!--</select>-->
                        <select id="selectpickerGroups" name="product_id[]" onChange="getBillingCycle(this.value)" required class="selectpicker w-100" data-style="btn-default">
                            @foreach($products as $product)
                                @if(count($product->productsWithTax) > 0)
                                <optgroup label="{{$product->category_name}}">
                                    @foreach($product->productsWithTax as $value)
                                      <option value="{{ $value->id }}" tax-rate="{{ $value->rate }}" tax-id="{{ $product->tax_id }}" tax-name="{{ $value->tax_name }}" >{{$value->product_name}}</option>
                                    @endforeach
                                </optgroup>
                                @endif
                            @endforeach
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <dl class="row mt-4">
                        ...
                        <dd class="col-6 text-end">
                            <div class="pull-right float-right">
                                {{$default_currency->prefix}} <span id="sub_total">0.00</span> {{$default_currency->code}}
                            </div>
                        </dd>
                    </dl>
                    <hr class="mx-n4">
                    ...
                    <dl class="row mt-4">
                        ...
                        <dd class="col-6 text-end">
                            <div class="pull-right float-right">
                                {{$default_currency->prefix}} <span id="tax_amount">0.00</span> {{$default_currency->code}} 
                            </div>
                        </dd>
                    </dl>
                    <hr class="mx-n4">
                    ...
                    <dl class="row mb-0">
                        ...
                        <dd class="col-6 fw-medium text-end text-heading mb-0">
                            <div class="total-due-today">
                                {{$default_currency->prefix}} <span id="total_amount">0.00</span> {{$default_currency->code}}
                                <input type="hidden" id="gstAmt" name="gstAmt">
                                <input type="hidden" id="totalAmt" name="totalAmt">
                                <input type="hidden" id="currency" name="currency" value="{{$default_currency->id}}">
                                <input type="hidden" id="tax_id" name="tax_id" value="{{$default_currency->id}}">
                            </div>
                        </dd>
                    </dl>
                    </h5>
...
<script>
  $(document).ready(function () {
    ...
    function getBillingCycle(id) {
        ...
        $.ajax({
            type: 'GET',
            url: "{{ url('admin/Orders/billingCycle') }}/" + id,
            success: function(res) {
                console.log(res);
    ...
    $('#selectpickerGroups').change(function(){
        ...
        $.ajax({
            type: 'GET',
            url: "{{ url('admin/Orders/getAddonProducts') }}/" + product_id,
            success: function(res) {
                if(res.length > 0){
                    $('#addons').html(res);
                }else{
   ...
   function calculateAmt() {
    ...
    $('.addon-checkbox:checked').each(function() {
        ...
        addonViewHtml += `<dl class="row mb-0">
                            ...
                            <dd class="col-6 text-end">
                                <div class="pull-right float-right">
                                    {{$default_currency->prefix}} <span>${numberFormat(addonPrice, 2)}</span> {{$default_currency->code}}
                                </div>
                            </dd>
                        </dl>`;
    });
