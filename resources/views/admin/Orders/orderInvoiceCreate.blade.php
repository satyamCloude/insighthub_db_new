  ...
  <div class="row">
    ...
    <div class="card">
      ...
      <form action="{{url('admin/Invoices/store')}}" method="post" enctype="multipart/form-data"> 
          ...
          <div class="row mb-4"> 
            <div class="col-md-3">
              ...
              <div class="input-group">
                ...
                <input type="number" name="invoice_number2" id="invoice_number2" class="form-control height-35 f-15" required autocomplete="off" value="{{$invoice_number2}}" readonly>
              </div>
            ...
            <div class="col-md-2">
              ...
              <input type="date" class="form-control startDate height-35 f-15" value="{{date('Y-m-d',strtotime($SelectedOrders->created_at))}}" name="issue_date" required/>
            </div>

            <div class="col-md-2">
              <label for="product_code" class="form-label">Due Date<span class="text-danger">*</span></label>
              <input type="date" class="form-control endDate height-35 f-15" value="{{date('Y-m-d',strtotime($SelectedOrders->created_at))}}" name="due_date" required/>
            </div>
            <div class="col-md-3">
        ...
        <div class="row mb-4"> 
          <div class="col-md-4">
            ...
            <select class="form-control select2" name="client_id" id="client_id" required>
              ...
              <option @if($SelectedOrders && $SelectedOrders->client_id == $Employee->id) selected @endif value="{{$Employee->id}}">{{$Employee->first_name}} ({{$Employee->id}})</option>
              @endforeach
            </select>
          </div>
          <div class="col-md-4">
              ...
              <div class="row mb-4">
                  <div class="col-md-4">
                      <select class="form-control select2" name="services_id" required id="services">
                          ...
                          @foreach($ProductNew as $Employee)
                              <option @if($SelectedOrders && $SelectedOrders->product_id == $Employee->id) selected @endif value="{{ $Employee->id }}" {{$Employee->id == $SelectedOrders->product_id ? 'selected':''}} >{{ $Employee->product_name }}</option>
                          @endforeach
                      </select>
                  </div>

                  <div class="col-md-4" id="get_service_orders">
                      <select name="services_type" id="services_type" required class="form-control select2" style="display: none;">
                          <option value="0" selected>--Select Order--</option>
                          @foreach($orders as $order)
                              <option @if($selected_order_id == $order->id) selected @endif {{$order->id == $id ? 'selected':''}} value="{{ $order->id }}">
                                  Order ID - {{ $order->id }}
                              </option>
                          @endforeach
                      </select>
                      ...
                      <table width="100%">
                        <tbody>
                          ...
                          <tr>
                            <td class="border-bottom-0 btrr-mbl btlr">
                              ...
                              placeholder="Item Name" autocomplete="off" @if($SelectedOrders && $SelectedOrders->order_type == 'client_order') value="{{$SelectedOrders->product_name}}  {{isset($adOnProductPrice) ? '+'.$adOnProductPrice->product_name :''}}" @endif  required>
                            </td>
                            <td class="border-bottom-0 d-block d-lg-none d-md-none">
                              <textarea class="form-control f-14 border-0 w-100 mobile-description form-control" name="item_summary"
                              placeholder="Enter Description (optional)">@if($SelectedOrders && $SelectedOrders->order_type == 'client_order') {!! $SelectedOrders->products_desc !!} @endif </textarea>
                            </td>
                            ...
                            <td class="border-bottom-0">
                              @if($SelectedOrders && $SelectedOrders->total_amt)
                              @php 
                              $adOnProductPrice1 = isset($adOnProductPrice) ?  $adOnProductPrice->price :'0'; 
                              $totalAmount = ($productAmount->price + $adOnProductPrice1); 
                          
                              @endphp
                              @else 0.00 
                              @endif
                                <input type="number" min="1" class="f-14 border-0 w-100 text-right cost_per_item form-control"
                                       value="{{isset($SelectedOrders->total_amt) ?$SelectedOrders->total_amt :'0:00'}}"
                                       name="cost_per_item" autocomplete="off" id="cost_per_item1" readonly required>
                            </td>

                            <td class="border-bottom-0">
                              <div class="select-others height-35 rounded border-0">
                                <div class="dropdown bootstrap-select show-tick select-picker type customSequence border-0">
                                  <select id="multiselect1" name="taxes" multiple="multiple" class="select-picker type customSequence border-0 multiselect" data-size="3" disabled>
                                   @foreach($Tax as $Taxs)
                                   <option data-rate="{{ $Taxs->rate }}" data-tax-text="{{ $Taxs->tax_name . ':' . $Taxs->rate }}" {{$SelectedOrders->tax_id == $Taxs->id ? 'selected':''}} value="{{$Taxs->id}}">{{ $Taxs->tax_name . ':' . $Taxs->rate }}
                                   </option>
                                   @endforeach
                                  </select>
                        ...
                        <tr class="d-none d-md-table-row d-lg-table-row">
                          <td colspan="3" class="dash-border-top bblr border-right-0">
                            ...
                            placeholder="Enter Description (optional)">@if($SelectedOrders && $SelectedOrders->order_type == 'client_order')  {{ strip_tags($SelectedOrders->products_desc) }} @endif </textarea>
                          </td>
       ...
       <div class="row mb-4"> 
        <div class="col-md-12">
          <label for="Add file"  class="form-label">Add File</label>
...
<script>
  $(document).ready(function () {
   ...
   var taxId = $('#multiselect1').val();
    var $select = $('#multiselect1');
  ...
  if (selectedOptions && selectedOptions.length > 0 && !isNaN(sub_total)) {
    selectedOptions.forEach(function (optionValue) {
      ...
      $taxTable.append($row);
      var calc_tax = parseFloat(calc_tax);
      var proAmount = parseFloat($('.amount').val());
      var totalAmount = calc_tax+proAmount;
  ...
  $('#services').change(function () {
    ...
    $('#services_type').change(function () {
      ...
      if(selectedServiceType == 0){
        selectedServiceType = $('#selectedOrderID').val();
      }else{
      ...
      $.ajax({
        ...
        data: {
          selectedServiceId: selectedServiceId,
          OrderID: selectedServiceType,
        },
        success: function (res) {
        console.log(res);

          if (res.data !== undefined && res.data !== null) {
              var selectedServiceTypeValue = res.data;
...
function calculateTotal(sectionCount) {
  ...
  $('.total').text(totalAmount2.toFixed(2));
      $('#final_total_amt').val(totalAmount2.toFixed(2));
