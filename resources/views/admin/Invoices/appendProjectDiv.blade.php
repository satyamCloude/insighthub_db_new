<div class="c-inv-desc-table item-row mt-4 w-100 d-lg-flex d-md-flex d-block">
    ...
    <tbody>
      ...
      <tr>
        <td class="border-bottom-0 btrr-mbl btlr">
          <input type="text" class="form-control f-14 border-0 w-100 item_name" id="item_name" name="item_name[]" placeholder="Item Name" autocomplete="off" value="{{isset($projectDetails) ? $projectDetails->project_name : ''}}">

          <input type="hidden" name="product_id[]" value="{{$projectDetails->id}}" autocomplete="off">
        </td>
        <td class="border-bottom-0 d-block d-lg-none d-md-none">
          <textarea class="form-control f-14 border-0 w-100 mobile-description form-control item_summary" id="item_summary" name="item_summary" placeholder="Enter Description (optional)">{{isset($projectDetails) ? $projectDetails->project_summary : ''}}</textarea>
        </td>
        ...
        <td class="border-bottom-0">
          <input type="number" min="1" class="f-14 border-0 w-100 text-right cost_per_item form-control"  name="cost_per_items[]" autocomplete="off" id="cost_per_item{{$numberCount}}" sectionCount="{{$numberCount}}"  required>
        </td>
          ...
          <div class="select-others height-35 rounded border-0">
            <div class="dropdown bootstrap-select show-tick select-picker type customSequence border-0">
              <select id="multiselect1{{$numberCount}}" name="taxes{{$numberCount}}[]" multiple="multiple" onchange="CalculateTax()" class="select-picker type customSequence border-0 multiselect" data-size="3">
                @foreach($Tax as $Taxs)
                <option data-rate="{{ $Taxs->tax_name }}" data-product-id{{$numberCount}}="{{$numberCount}}" data-tax-value="{{ $Taxs->rate }}" data-tax{{$numberCount}}="{{ $Taxs->tax_name . ':' . $Taxs->rate }}" value="{{$Taxs->id}}"  >{{ $Taxs->tax_name . ':' . $Taxs->rate }} </option>
                @endforeach
              </select>
  ...
  onclick="removeSection2(this)"><i class="fa fa-times-circle f-20 text-lightest"></i></a> 
   &nbsp;&nbsp;&nbsp;
</div>
