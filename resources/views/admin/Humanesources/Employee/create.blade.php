        ...
        <div class="card">
          <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row">
            ...
            <div class="action-btns">
              <a href="{{url('admin/Employee/home')}}" class="btn btn-label-primary me-3">
    ...
    <div class="card-body">
        ...
        <div class="row mb-4"> 
            ...
            <div class="col-md-4">
                ...
                <input type="text" class="form-control" name="first_name" placeholder="Enter First Name" value="{{ old('first_name') }}" required/>
                @error('first_name')
                    <div class="error">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="row mb-4"> 
            <div class="col-md-6">
                ...
                <input type="email" class="form-control" id="personal_email" name="email" placeholder="abc@gmail.com" value="{{ old('email') }}" required/>
                @error('email')
                    <div class="error">{{ $message }}</div>
                @enderror
            </div>
            <div class="col-md-6">
                ...
                <input type="tel" name="phone_number" id="phone_number" class="form-control" required placeholder="+1234567890" pattern="[0-9+]{1,10}" maxlength="10" value="{{ old('phone_number') }}" oninput="this.value = this.value.replace(/[^0-9+]/g, '').slice(0, 10);">
                @error('phone_number')
                    <div class="error">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="row mb-4"> 
            ...
            <div class="col-md-6">
                ...
                <input type="date" class="form-control" name="marriage_anniversary" value="{{ old('marriage_anniversary') }}"/>
        ...
        <div class="row mb-4"> 
            ...
            <div class="col-md-6">
                ...
                <input type="number" class="form-control" id="net_salary" name="net_salary" placeholder="1000" value="{{ old('net_salary') }}" required min="1000"/>
                @error('net_salary')
                    <div class="error">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="row mb-4"> 
                  <div class="col-md-6">
                        <div class="avatar-preview">
                           <img id="uploadedProfile" width="100" height="100"  src="{{url('public/images/21104.png')}}" alt="Preview" />
                        </div>
                  </div>
                  <div class="col-md-6">
        ...
        <textarea class="form-control" id="address" name="address" rows="3" required>{{ old('address', $EmployeeDetail->address ?? '') }}</textarea>
        @error('address')
            <div class="error">{{ $message }}</div>
        @enderror
    </div>
        ...
        <div class="row mb-4"> 
            <div class="col-md-6">
                ...
                <select name="job_role_id" class="form-control select2" required>
                    <option value="0">Select</option>
                    @foreach($Role as $Rol)
                        <option value="{{ $Rol->id }}" {{ old('job_role_id') == $Rol->id ? 'selected' : '' }}>{{ $Rol->name }}</option>
                    @endforeach
                </select>
                @error('job_role_id')
                    <div class="error">{{ $message }}</div>
            ...
            <div class="col-md-6 select2-primary">
                ...
                <select name="jobrole_id" class="form-control select2" required>
                    <option value="0">Select</option>
                    @foreach($Jobrole as $Per)
                        <option value="{{ $Per->id }}" {{ old('jobrole_id') == $Per->id ? 'selected' : '' }}>{{ $Per->name }}</option>
                    @endforeach
                </select>
                @error('jobrole_id')
                    <div class="error">{{ $message }}</div>
        ...
        <div class="row mb-4"> 
            <div class="col-md-6">
                ...
                <br>
                <input type="checkbox" class="form-check-input input-filter" name="team_lead" id="team_lead" {{ old('team_lead') ? 'checked' : '' }} />
                @error('team_lead')
                    <div class="error">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="row mb-4"> 
            <div class="col-md-6">
                ...
                <select name="weekly_off_id" id="weekly_off_id" class="form-control select2" required>
                    ...
                    @foreach($Weekly as $Week)
                        <option value="{{ $Week->id }}" {{ old('weekly_off_id') == $Week->id ? 'selected' : '' }}>{{ $Week->name }}</option>
                    @endforeach
                </select>
                @error('weekly_off_id')
                    <div class="error">{{ $message }}</div>
            ...
            <div class="col-md-6 ">
                ...
                <select name="additional_week_off_first" class="form-control select2">
                    ...
                    @foreach($Weekly as $Week)
                        <option value="{{ $Week->id }}" {{ old('additional_week_off_first') == $Week->id ? 'selected' : '' }}>{{ $Week->name }}</option>
                    @endforeach
                </select>
                @error('additional_week_off_first')
                    <div class="error">{{ $message }}</div>
                @enderror
            </div>
        </div>
        <div class="row mb-4"> 
            <div class="col-md-6">
                ...
                <select name="additional_week_off_second" class="form-control select2">
                    <option value="0">Select day</option>
                    @foreach($Weekly as $Week)
                        <option value="{{ $Week->id }}" {{ old('additional_week_off_second') == $Week->id ? 'selected' : '' }}>{{ $Week->name }}</option>
                    @endforeach
                </select>
                @error('additional_week_off_second')
                    <div class="error">{{ $message }}</div>
                @enderror
            </div>
            <div class="col-md-6 ">
                ...
                <select name="additional_week_off_third" class="form-control select2">
                    <option value="0">Select day</option>
                    @foreach($Weekly as $Week)
                        <option value="{{ $Week->id }}" {{ old('additional_week_off_third') == $Week->id ? 'selected' : '' }}>{{ $Week->name }}</option>
                    @endforeach
                </select>
                @error('additional_week_off_third')
                    <div class="error">{{ $message }}</div>
                @enderror
            </div>
        </div>
        <div class="row mb-4"> 
            <div class="col-md-6">
                ...
                <select name="additional_week_off_fourth" class="form-control select2">
                    <option value="0">Select day</option>
                    @foreach($Weekly as $Week)
                        <option value="{{ $Week->id }}" {{ old('additional_week_off_fourth') == $Week->id ? 'selected' : '' }}>{{ $Week->name }}</option>
                    @endforeach
                </select>
                @error('additional_week_off_fourth')
                    <div class="error">{{ $message }}</div>
                @enderror
            </div>
            <div class="col-md-6 ">
                ...
                <select name="status" class="form-control select2" required>
                    ...
                    <option value="2" {{ old('status') == '2' ? 'selected' : '' }}>Resigned</option>
        ...
        <div class="row mb-4"> 
            <div class="col-md-6">
                ...
                <select name="working_type_id" class="form-control select2" required>
                    <option value="1" {{ old('working_type_id') == '1' ? 'selected' : '' }}>Work From Office</option>
                    <option value="2" {{ old('working_type_id') == '2' ? 'selected' : '' }}>Work From Home</option>
                </select>
                @error('working_type_id')
                    <div class="error">{{ $message }}</div>
                @enderror
            </div>
            <div class="col-md-12">
                ...
                <div class="editor-container">
                    <div class="full-editor geteditor">{!! old('signature') !!}</div>
            ...
            <div class="col-md-12">
                ...
                <select name="shift_id" class="form-control select2" required>
                    @foreach($TimeShift as $Time)
                        <option value="{{ $Time->id }}" {{ old('shift_id') == $Time->id ? 'selected' : '' }}>
                            {{ $Time->shift_name }} <b>-</b> {{ \Carbon\Carbon::parse($Time->StartTime)->format('H:i') }} <b>To</b> {{ \Carbon\Carbon::parse($Time->EndTime)->format('H:i') }}
                        </option>
                    @endforeach
                </select>
                @error('shift_id')
        ...
        <div class="row mb-4"> 
            <div class="col-md-12">
                ...
                <div class="editor-container">
                    <div class="full-editor geteditor">{!! old('kra') !!}</div>
        ...
        <div class="row mb-4"> 
            <div class="col-md-6">
                ...
                <input type="email" class="form-control" id="login_email" name="login_email" value="{{ old('login_email') }}" required />
                @error('login_email')
                    <div class="error">{{ $message }}</div>
                @enderror
            </div>
    ...
    <div class="card-footer">
        <div class="row mb-4"> 
            <div class="col-md-6 text-end">
                <a href="{{ url('admin/Employee/home') }}" type="button" class="btn btn-outline-danger">Discard</a>
            </div>
            <div class="col-md-6">
