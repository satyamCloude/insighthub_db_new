                              ...
                              @foreach($attendance2 as $key => $row)
                                ...
                                // Calculate total working seconds based on punch_in and punch_out times
                                  ...
                                  <tr>
                                      ...
                                      <td>{{ $row->punch_out ?? '-' }}</td>
                                      <td>{{ $totalhours }}</td>
                                      <td>{{ $row->break_time ? sprintf('%02dh %02dmin %02dsec', floor($row->break_time / 60), $row->break_time % 60, 0) : '0h 00min 00sec' }}</td>
                                      <td>{{ $row->overtime ?? '0h 00min 00sec' }}</td>
                                      <td style="color:green">{{ $totalProductionHours }}</td>
                                      <!-- <td>{{ \Carbon\Carbon::parse($user->ts_working_hrs)->format('G:i').'Hr' }}</td> -->
   ...
   */
   ...
   var currentYear = new Date().getFullYear();
       ...
       function fetchData() {
           ...
           $.ajax({
               url: "{{ url('admin/Attendence/filterAttendance/'.$id) }}",
               method: 'GET',
