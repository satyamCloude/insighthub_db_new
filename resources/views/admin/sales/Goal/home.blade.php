   ...
   <div class="card">
      <div class="row">
         ...
         <div class="col-md-6 text-end">
            <a href="{{url('admin/Goal/home')}}" class="btn btn-warning mt-3 m-3"><i class="fas fa-sync-alt"></i></a>
            <a href="{{url('admin/Goal/add')}}" class="btn btn-primary mt-3 m-3">Add Goal</a>
         </div>
      </div>
            ...
            <table class="dt-responsive table dataTable dtr-column" id="DataTables_Table_3" aria-describedby="DataTables_Table_3_info">
               ...
               <tbody id="result">
                  ...
                  <tr class="odd">
                     <td>{{ $key + 1 }} </td>
                     <td>
                        <img class="rounded-circle" style="margin-right: 15px;margin-top: 10px;" src="{{isset($user->profile_img) ? $user->profile_img : url('public/images/21104.png')}}" height="30" width="30" alt="User avatar" /><a href="{{ url('admin/Goal/view/'.$user->id.'/'.$user->employee_id) }}">{{$user->first_name }}</a>
                        <div style="font-size:12px;margin-left: 46px;margin-top: -11px;">{{$user->job_name}}</div>
                     </td>
                     <!-- <td>@if($user && $user->job_name) {{ $user->job_name }} @endif</td> -->
                     <td>@if($user && $user->goal_value) {{ $user->goal_value }} @endif</td>
                     <td>
                        <span class="{{$textClass}}">
                        @if($user && $user->archieved_value) {{ $user->archieved_value }} @endif
                     </td>
                     </span>
                     <td>
                        @if($user && $user->goal_value != 0)
                     ...
                     <td>
                        <div class="btn-group">
                           ...
                           <ul class="dropdown-menu dropdown-menu-end" style="">
                              <li><a class="dropdown-item" href="{{ url('admin/Goal/view/'.$user->id.'/'.$user->employee_id) }}">View</a></li>
                              @if($user->goalCreateMonth >= date('m'))
