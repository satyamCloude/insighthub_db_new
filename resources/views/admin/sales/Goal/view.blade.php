                  ...
                  <table class="dt-responsive table dataTable dtr-column" id="DataTables_Table_3" aria-describedby="DataTables_Table_3_info">
                     ...
                     <tbody id="result">
                        ...
                        <tr class="odd">
                           ...
                           <td class="goal_value">
                              @if($user && $user->goal_value) {{ $user->goal_value }} @endif
                           </td>
                           ...
                           <td>
                              <div class="btn-group">
                                 ...
                                 <ul class="dropdown-menu dropdown-menu-end">
                                    ...
                                    @endif
                                    <li>
                                       <button class="dropdown-item delete_debtcase" data-url="{{ url('admin/Goal/delete/'.$user->id) }}" data-id="{{ $user->id }}">Delete</button>
                                    </li>
                                 </ul>
                              </div>
                           </td>
...
<script>
  ...
  $(document).ready(function () {
    ...
    function updateTable(data) {
      ...
      if (data.data.goalData.length > 0) {
        $.each(data.data.goalData, function (key, user) {
          ...
          row += '<a href="{{ url("admin/Goal/view") }}/' + user.id + '">' + user.first_name + '</a>';
          ...
          row += '<td>';
           ...
           row += '<li><button class="dropdown-item delete_debtcase" url="{{ url("admin/Goal/delete") }}/' + user.id + '" id="' + user.id + '">Delete</button></li>';
           row += '</ul>';
           row += '</div>';
           row += '</td>';
           row += '</tr>';
    ...
    $('.fromDate, .toDate').on('change', function () {
        ...
        $.ajax({
          type: 'GET',
          url: "{{ url('admin/Goal/get_Goal_data_date_wise') }}",
          data: {
            fromDate: fromDate,
            toDate: toDate,
            Empid: Empid,
...
<script>
   ...
   // Handle the change event of the select elements
    ...
    function fetchData() {
       ...
       $.ajax({
         url: "{{ url('admin/Goal/get_Goal_data') }}",
         method: 'GET',
