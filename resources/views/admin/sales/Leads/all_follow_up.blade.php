       ...
       <tbody>
        <tr class="group">
          <td colspan="10" style="background-color:#dbdade">Today</td>
              ...
              <tr>
                <td>{{$Followup->id}}</td>         
                <td><a href="{{url('admin/Leads/view/'.$Followup->leads_id)}}">{{$Followup->leads_id}}</a></td>         
                <td>
                    @php
...
<script>
 ...
 $(document).ready(function () {
  ...
  function fetchData() {
    ...
    $.ajax({
      url: "{{ url('admin/Leads/get_followups_yeardata') }}",
      method: 'GET',
...
function EditFollowUp(id) {
  ...
  $('#statusFilter').on('change', fetchData1);
   ...
   function fetchData1() {
    ...
    $.ajax({
      url: "{{ url('admin/Leads/get_follow_up_type') }}",
      method: 'GET',
