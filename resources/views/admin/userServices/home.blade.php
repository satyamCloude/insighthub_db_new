...
<div class="container-xxl flex-grow-1 container-p-y">
  <h4 class="py-3 mb-4"><span class="text-muted fw-light"> User Services /</span> Home</h4>
                    ...
                    <div class="card-body">
                      <div class="d-flex align-items-start justify-content-between">
                        <div class="content-left">
                          <span>Declined</span>
                          <div class="d-flex align-items-center my-2">
                            <h3 class="mb-0 me-2">{{$declined}}</h3>
                            <!-- <p class="text-success mb-0"></p> -->
                          </div>
                          <p class="mb-0">Total</p>
                        </div>
  ...
  <div class="card">
    <div class="row">
      <div class="col-md-6">
          <h5 class="card-header">User Services List</h5>
      </div>
      ...
      <div class="card-datatable table-responsive">
        <div id="DataTables_Table_3_wrapper" class="dataTables_wrapper dt-bootstrap5">
          ...
          <table class="dt-responsive table dataTable dtr-column" id="DataTables_Table_3" aria-describedby="DataTables_Table_3_info">
            ...
            <tbody>
              @if(count($orderByServices) > 0)
              @foreach($orderByServices as $key=> $BareMetals)
              <tr class="odd">
                  <td>{{ $key+1 }} </td>
                  <td>
                    <img 
                            ...
                            src="{{$BareMetals->profile_img}}"
                            height="30"
                            width="30"
                            alt="User avatar" />{{$BareMetals->first_name }}<div style="font-size:12px;margin-left: 46px;margin-top: -11px;">{{$BareMetals->email}}</div>
                          </td>
                  <td>@if($BareMetals && $BareMetals->Pproduct_name) {{ $BareMetals->Pproduct_name }} @endif</td>
                  <td>@if($BareMetals && $BareMetals->public_ip) {{ $BareMetals->public_ip }} @endif</td>
                  <td>@if($BareMetals && $BareMetals->category_name) {{ $BareMetals->category_name }} @endif </td>
                  <td>@if($BareMetals && $BareMetals->created_at) {{ date('Y-m-d',strtotime($BareMetals->created_at)) }} @endif</td>
                   <td>
                   @switch($BareMetals->order_status)
                         @case('0')
