               ...
               <div class="border-grey p-4 border-top-0 rounded-bottom">
                  <div class="row">
                     <div class="col-md-11">
                        ...
                        <p class="mb-4 mt-2 f-14 text-dark-grey">Enabling this feature will send code on your email account
                           <b>{{ Auth::user()->login_email}}</b> for log in.
                        </p>
                        @php
                        $data = App\Models\OneTimeSetup::where('user_id',Auth::user()->id)->first();
                        @endphp
                        @if($data && $data->is_authentication_enabled)
                        ...
                        <div id="qrcode">
                           <h6>Setup Using Google Authenticator</h6>
                           <h6>@if(Auth::user()->google2fa_enabled){{ Auth::user()->google2fa_enabled ? Auth::user()->first_name.' has enabled' : Auth::user()->first_name.' has not enabled' }} @endif two-factor authentication.</h6>
                           ...
                           @endif
                           <!--  @if(Auth::user()->google2fa_enabled)
                              <a href="{{url('admin/SecuritySettings/2fa/disable?id='.Auth::user()->id)}}" >
                                  <button class="btn btn-danger">Disable</button>
                              </a>
                              @else
                              <a href="{{url('admin/SecuritySettings/2fa/enable?id='.Auth::user()->id)}}" >
                              ...
                              <div class="col-lg-4 mt-3">
                                    <div class="form-check">
                                       <input 
                                          ...
                                          value="0" 
                                          autocomplete="off" 
                                          {{ isset($passwordDays) && $passwordDays->password_security_days == 0 ? 'checked' : '' }}>
                                       <label 
                              ...
                              <div class="col-lg-4 mt-3">
                                    <div class="form-check">
                                       <input 
                                          ...
                                          value="1" 
                                          autocomplete="off" 
                                          {{ isset($passwordDays) && $passwordDays->password_security_days == 1 ? 'checked' : '' }}>
                                       <label 
                                          class="form-check-label form_custom_label text-dark-grey pl-2 mr-3 justify-content-start cursor-pointer checkmark-20 text-wrap" 
...
<script>
   ...
   document.querySelectorAll('input[name="password_security_days"]').forEach((radio) => {
         radio.addEventListener('change', function() {
            ...
            fetch("{{ url('admin/updatePasswordDays') }}", {
                  method: "POST",
                  headers: {
