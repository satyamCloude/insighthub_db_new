...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="row mt-4 m-4">
        ...
        <div class="col-md-8 text-end ">
           ...
           <a href="{{url('admin/LogActivity/invoice')}}"class="btn btn-primary">Invoice</a>
           <a href="{{url('admin/LogActivity/lead')}}"class="btn btn-light">Lead</a>
        </div>
    </div>
    <div class="card  m-4">
        ...
        <div id="DataTables_Table_3_wrapper" class="dataTables_wrapper dt-bootstrap5">
          <div class="row bg-label-dark text-dark">
            ...
            <div class="col-sm-12 col-md-6 d-flex justify-content-center justify-content-md-end">
              <div id="DataTables_Table_3_filter" class="dataTables_filter">
                  ...
                  <a href="{{url('admin/LogActivity/ticket')}}"class="btn btn-warning mt-3 m-3"><i class="fas fa-sync-alt"></i></a>
                </form>
              </div>
            </div>
          </div>
          <table class="dt-responsive table dataTable dtr-column" id="DataTables_Table_3" aria-describedby="DataTables_Table_3_info">
            ...
            <tbody id="result">
             ...
             @foreach($LogActivity as $key=> $user)
              <tr class="odd">
                  ...
                  <td>{{ $user->subject }}f</td>
                  <td>{{$user->user_id == 1 ? 'Admin ID' : 'Client ID' }} {{ $user->user_id }}<br><small style="color:#b5b2b2;">{{ $user->email }}</small></td>
                  <td>{{ $user->ip }}</td>
              </tr>
              @endforeach
              @else
            ...
            <div class="row">
              ...
              <div class="col-md-6 text-right">{{$LogActivity->links()}}</div>
            </div>
        </div>
      </div>
  </div>
