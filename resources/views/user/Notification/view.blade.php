      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header sticky-element bg-label-secondary d-flex justify-content-sm-between align-items-sm-center flex-column flex-sm-row">
            <h5 class="card-title mb-sm-0 me-2">View</h5>
            <div class="action-btns">
              <form action="{{ route('razorpay.payment.store') }}" method="POST" >
                 @csrf
                 <input type="hidden" name="invoice_id" value="{{$invoice_id->id}}">
                 <input type="hidden" name="order_id" value="{{$order_id->id}}">
                 <input type="hidden" name="quotes_id" value="{{$Quotes->id}}">
                 <input type="hidden" name="user_id" value="{{Auth::user()->id}}">
                 <input type="hidden" id="r_payment_id" name="razorpay_payment_id">
              </form>
              @if($Quotes->is_payment_recieved == 0)
              <button type="button" class="btn btn-label-primary" id="rzp-button1">Pay Now</button>
              @endif
            </div>
          </div>
                    ...
                    <tbody>
                        <tr>
                            <td align="end" height="30" colspan="2">
                                <img src="{{$Quotes->signature}}" class="mt-2 mb-2" height="70" width="15%">
                            </td>
                        </tr>
                        <tr>
                            <td>Note</td>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var amountInPaise = Math.max(Math.floor(totalAmountNew * 100), 100);

        // Set the data-amount attribute dynamically
        $('#AmountR').attr('data-amount', amountInPaise.toString());
        ...
        var rzp = new Razorpay({
            ...
            currency: 'INR', // Adjust the currency code as needed
            name: 'Cloudtechtiq.com',
            description: 'Rozerpay',
            image: '{{ url("public/logo/favicon.png") }}',
            prefill: {
                name: 'name',
                email: 'email',
            },
           ...
           handler: function (response) {
                if (response.razorpay_payment_id) {
                    ...
                    // You can handle errors and display a message to the user
                    alert('Payment failed or canceled.');
                }
            }
        });

        // Click event for the Pay Now button
