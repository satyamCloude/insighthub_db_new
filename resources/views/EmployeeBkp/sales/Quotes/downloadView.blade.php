            ...
            <div class="invoice-table-wrapper">
                <table width="100%">
                    <tbody><tr class="inv-logo-heading">
                        <td class="tthead"><img src="@if($Quotes && $Quotes->companylogo) {{$Quotes->companylogo}} @endif" alt="Worksuite" id="logo"></td>
                    ...
                    <tr class="inv-num">
                        <td width="61%" class="f-14 text-dark">
                            <p class="mt-3 mb-0"><br>
                             @if($Quotes && $InvoiceSettings->show_client_company_name ==1)
                             {{$Quotes->company_name}}
                             @endif<br> 
                             @if($Quotes)

                             {{$Quotes->contact_no}} <br>
                             {{$Quotes->billing_address}}                             @endif
                             </p> 
                        </td>
                        <td align="right">
                            <table class="inv-num-date text-dark f-13 mt-3 marg" width="100%">
                                <tbody>
                                    <tr>
                                        ...
                                        Invoice Number</td>
                                        <td class="border-left-0"> @if($Quotes){{$Quotes->invoice_number1}} {{$Quotes->invoice_number2}}  @endif</td>
                                    </tr>
                                <tr>
                                    <td class="bg-light-grey border-right-0 f-w-500">Invoice Date</td>
                                    <td class="border-left-0">@if($Quotes){{$Quotes->date_created}}@endif</td>
                                </tr>
                                <tr>
                                    <td class="bg-light-grey border-right-0 f-w-500">Due Date</td>
                                    <td class="border-left-0">@if($Quotes){{$Quotes->valid_until}}@endif</td>
                                </tr>
                            </tbody>
                        </table>
                    </td>
                ...
                <tbody><tr class="inv-unpaid">
                    <td class="f-14 text-dark">
                         <p class="mb-0 text-left">
                          ...
                          @if($Quotes && $InvoiceSettings->show_client_name ==1)
                          {{ $Quotes->first_name }} {{ $Quotes->last_name }}
                          @endif<br>

                          @if($Quotes && $InvoiceSettings->show_client_email ==1)
                          {{ $Quotes->email }}
                          @endif<br>

                          @if($Quotes && $InvoiceSettings->show_client_phone ==1)
                          {{ $Quotes->phone_number }}
                          @endif<br>

                          @if($Quotes && $InvoiceSettings->show_client_company_address ==1)
                          {{ $Quotes->address_1 }}<br>
                          {{ $Quotes->address_2 }}<br>
                          @endif
                          @if($Quotes)

                          {{ $Quotes->pincode }}                          @endif
<br>

                        </p>
                    </td>
                        <td align="right" class="mt-2 mt-lg-0 mt-md-0">
                            @if ($Quotes && $Quotes->is_payment_received == 0)
                            <span class="unpaid rounded f-15">Unpaid</span>
                            @elseif ($Quotes && $Quotes->is_payment_received == 1)
                        ...
                        <td colspan="2">
                            <table class="inv-detail f-14 table-responsive-sm" width="100%">
                                <tbody>
                                    ...
                                    @php $Tax =  \App\Models\TaxSetting::whereIn('id', explode(',',$Quote->Caltax))->get() @endphp
                                    <tr class="text-dark font-weight-semibold f-13">
                                      ...
                                      <td align="right">@foreach($Tax as $tax) {{$tax->tax_name}} {{$tax->rate}}% @if(!$loop->last) , @endif @endforeach</td>
                                                        ...
                                                        <td colspan="5" class="border-bottom-0">
                                                            rty
                                                                                            ...
                                                                                            <table class="inv-note" width="100%">
                                                                                                ...
                                                                                                <tr>
                                                                                                    <td width="100%"></td>
                                                                                                    <td><img src="@if($Quotes){{$Quotes->signature}} @endif"  class="mt-2 mb-2" height="70" width="15%"></td>
                                                                                                </tr>
                                                                                                <tr>
                                                                                                    <td>Note</td>
                                                                                                ...
                                                                                                <tr>
                                                                                                    <td style="vertical-align: text-top">
                                                                                                        <p class="text-dark-grey">@if($Quotes){{$Quotes->subject}} @endif </p>
                                                                                                    </td>
