                    ...
                    <div class="email-filters py-2">
                      ...
                      <ul class="email-filter-folders list-unstyled mb-4">
                       <!--  <li class=" d-flex justify-content-between" data-target="inbox">
                        ...
                        <li class="@if(session('tab') == 'sent') active @endif d-flex justify-content-between" onclick="Sent({{Auth::user()->id}})" data-target="sent">
                          <a href="javascript:void(0);" class="d-flex flex-wrap align-items-center">
                            <i class="ti ti-send ti-sm"></i>
                            <span class="align-middle ms-2">Sent</span>
                          </a>
                          <div class="badge bg-label-success rounded-pill badge-center">{{$Sent}}</div>
                        </li>
                        <li class="@if(session('tab') == 'draft') active @endif d-flex justify-content-between" data-target="draft" onclick="DraftMails({{Auth::user()->id}})">
                          <a href="javascript:void(0);" class="d-flex flex-wrap align-items-center">
                            <i class="ti ti-file ti-sm"></i>
                            <span class="align-middle ms-2">Draft</span>
                          </a>
                          <div class="badge bg-label-primary rounded-pill badge-center">{{$Draft}}</div>
                        </li>
                        <li class="@if(session('tab') == 'star') active @endif d-flex justify-content-between" data-target="starred" onclick="StarMails({{Auth::user()->id}})">
                          <a href="javascript:void(0);" class="d-flex flex-wrap align-items-center">
                            <i class="ti ti-star ti-sm"></i>
                            <span class="align-middle ms-2">Starred</span>
                          </a>
                          <div class="badge bg-label-warning rounded-pill badge-center">{{$Star}}</div>
                        </li>
                       <!--  <li class="d-flex align-items-center" data-target="spam">
                        ...
                        <li class="@if(session('tab') == 'trash') active @endif d-flex justify-content-between" data-target="trash"  onclick="Trash({{Auth::user()->id}})">
                          <a href="javascript:void(0);" class="d-flex flex-wrap align-items-center">
                            <i class="ti ti-trash ti-sm"></i>
                            <span class="align-middle ms-2">Trash</span>
                          </a>
                          <div class="badge bg-label-danger rounded-pill badge-center">{{$Trash}}</div>
                        </li>
                        <li class="@if(session('tab') == 'schedule') active @endif d-flex justify-content-between" data-target="schedule"  onclick="Schedule({{Auth::user()->id}})">
                          <a href="javascript:void(0);" class="d-flex flex-wrap align-items-center">
                            <i class="ti ti-clock ti-sm"></i>
                            <span class="align-middle ms-2">Schedule</span>
                          </a>
                          <div class="badge bg-label-info rounded-pill badge-center">{{$schedule}}</div>
                        </li>
                      </ul>
                    ...
                    <div class="shadow-none border-0">
                      <div class="emails-list-header p-3 py-lg-3 py-2">
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                          ...
                          <div class="d-flex align-items-center mb-0 mb-md-2">
                            <a href="{{url('Employee/MassMail/home')}}" class="ti ti-rotate-clockwise ti-sm rotate-180 scaleX-n1-rtl cursor-pointer email-refresh me-2"></a>
                      ...
                      <div class="email-list pt-0">
                        <ul class="list-unstyled m-0">
                          @if(count($MassMail)  > 0)
                             @foreach($MassMail as $key=> $Mass)
                             <li class="email-list-item " ondblclick="ViewEmail({{$Mass->id}})">
                               <div class="d-flex align-items-center">
                                 ...
                                 <i class="email-list-item-bookmark ti ti-star ti-sm d-sm-inline-block d-none cursor-pointer ms-2 me-3 @if($Mass && $Mass->star == 1) text-warning @endif" onclick="StarUpdate({{$Mass->id}})"></i>
                                 <ul class="list-unstyled m-0 d-flex align-items-center avatar-group my-3">
                                    @php $senduser =  \App\Models\User::whereIn('id', explode(',',$Mass->to_id))->where('type',2)->get() @endphp
                                 ...
                                 <div class="email-list-item-content ms-2 ms-sm-0 me-2">
                                   <!-- <span class="h6 email-list-item-username me-2">{{$Mass->first_name}}</span> -->
                                   <span class="email-list-item-subject d-xl-inline-block d-block me-2">{{$Mass->subject}}</span>
                                 </div>
                                 <div class="email-list-item-meta ms-auto d-flex align-items-center">
                                   <span class="email-list-item-label badge badge-dot @if($Mass && $Mass->status == '4') bg-danger @else bg-success @endif d-none d-md-inline-block me-2" data-label="important"></span>
                                   <small class="email-list-item-time text-muted">{{$Mass->created_at->format('g:i A')}}</small>
                                   <ul class="list-inline email-list-item-actions text-nowrap">
                                     ...
                                     <li class="list-inline-item email-delete" onclick="Delete({{$Mass->id}})"><i class="ti ti-trash ti-sm"></i></li>
                                   </ul>
                                 </div>
                               </div>
                             </li>
                  ...
                  <div class="modal-dialog m-0 me-md-4 mb-4 modal-lg">
                    <div class="modal-content p-0">
                      ...
                      <div class="modal-body flex-grow-1 pb-sm-0 p-4 py-2">
                        <form class="email-compose-form" action="{{url('Employee/MassMail/store')}}" method="POST">
                          @csrf
                          ...
                          <div class="email-compose-to d-flex justify-content-between align-items-center">
                              ...
                              <div class="select2-primary border-0 shadow-none flex-grow-1 mx-2">
                                  <select class="select2 select-email-contacts form-select CallImage" id="emailContacts" name="to_id[]" multiple required>
                                      <option data-avatar="{{url('public/logo/check.png')}}" value="0">Select All</option>
                                      @foreach($Client as $client)
                                          @php
                                              if(isset($client->profile_img))
                                                  ...
                                                  $url = url('public/images/emp_proa4NN.jpg');
                                          @endphp
                                          <option data-avatar="{{ $url }}" value="{{ $client->id }}">{{ $client->first_name }}-({{ $client->id }})</option>
                                      @endforeach
                                  </select>
                              </div>
                          ...
                          <div
                            class="email-compose-actions d-flex justify-content-between align-items-center mt-3 mb-3">
                             <div class="d-flex align-items-center">
                                <div class="btn-group">
                                    <button type="submit" id="send" name="send" value="public" class="btn btn-primary">
                            ...
                            <div class="d-flex align-items-center">
                              <div class="dropdown">
                                ...
                                <ul class="dropdown-menu" aria-labelledby="dropdownMoreActions">
                                  <li><button type="button" class="dropdown-item" onclick="copy(value)" value="{{$Name}}">{{$Name}} <i class="fa-solid fa-copy"></i></button></li>
                                  <li><button type="button" class="dropdown-item" onclick="copy(value)" value="{{$Email}}">{{$Email}} <i class="fa-solid fa-copy"></i></button></li>
                                </ul>
                              </div>
   ...
   @if(session('tab'))
    <script>
        window.onload = function() {
            if("{{ session('tab') }}" == 'draft') {
                DraftMails({{ Auth::user()->id }});
            }

            if("{{ session('tab') }}" == 'star') {
                StarMails({{ Auth::user()->id }});
            }

            if("{{ session('tab') }}" == 'trash') {
                Trash({{ Auth::user()->id }});
            }

            if("{{ session('tab') }}" == 'schedule') {
                Schedule({{ Auth::user()->id }});
            }
...
<script>
 ...
 var datetimeInput = $('#html5-datetime-local-input');
  ...
  function Sent(user_id){
     $.ajax({
                url: "{{ url('Employee/MassMail/SentMails') }}",
                method: 'GET',
   ...
   function edit(id){
     $.ajax({
                url: "{{ url('Employee/MassMail/Edit') }}",
                method: 'GET',
                data: { id: id },
                success: function (data) {
                    if (data.success) {
 ...
 $.ajax({
                url: "{{ url('Employee/MassMail/Show') }}",
                method: 'GET',
                data: { id: id },
                success: function (data) {
                    if (data.length > 0) {
 ...
 $.ajax({
                url: "{{ url('Employee/MassMail/DraftMails') }}",
                method: 'GET',
                data: { id: id },
                success: function (data) {
                    if (data.length > 0) {
    ...
    function Schedule(id) {
              $.ajax({
                url: "{{ url('Employee/MassMail/Schedule') }}",
                method: 'GET',
                data: { id: id },
                success: function (data) {
                    if (data.length > 0) {
 ...
 $.ajax({
                url: "{{ url('Employee/MassMail/StarMails') }}",
                method: 'GET',
                data: { id: id },
                success: function (data) {
                    if (data.length > 0) {
    ...
    function StarUpdate(id) {
        $.ajax({
          url: "{{ url('Employee/MassMail/StarUpdate') }}",
          method: 'GET',
          data: { id: id },
          success: function () {
               location.reload();
     ...
     function SendM(id) {
        bootbox.confirm({
          ...
          callback: function (result) {
              if (result) {
                   $.ajax({
                    url: "{{ url('Employee/MassMail/SendM') }}",
                    method: 'GET',
                    data: { id: id },
                    success: function () {
                         location.reload();
     ...
     function Delete(id) {
        bootbox.confirm({
          ...
          callback: function (result) {
              if (result) {
                   $.ajax({
                    url: "{{ url('Employee/MassMail/Delete') }}",
                    method: 'GET',
                    data: { id: id },
                    success: function () {
                         location.reload();
    ...
    function Restore(id) {
      ...
      bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
               $.ajax({
                  url: "{{ url('Employee/MassMail/Restore') }}",
                  method: 'GET',
                  data: { id: id },
                  success: function () {
                       location.reload();
    ...
    function ForceDelete(id) {
      bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
              $.ajax({
                url: "{{ url('Employee/MassMail/ForceDelete') }}",
                method: 'GET',
                data: { id: id },
                success: function () {
                     location.reload();
   ...
   function Trash(id) {
          $.ajax({
                url: "{{ url('Employee/MassMail/Trash') }}",
                method: 'GET',
                data: { id: id },
                success: function (data) {
                    if (data.length > 0) {
...
function LoadMore(button) {
    ...
    $.ajax({
        type: "get",
        data: { type: type },
        url: "{{ url('Employee/MassMail/LoadMore') }}?page=" + page,
        success: function (data) {
            if (data != '') {
                $('.email-list').html(data);
