  ...
  <div class="card">
    <div class="row">
      ...
      <div class="col-md-6 text-end">
          <a href="{{url('Employee/BareMetal/home')}}" class="btn btn-warning mt-3 m-3"><i class="fas fa-sync-alt"></i></a>
          @if($RoleAccess[array_search('BareMetal', array_column($RoleAccess, 'per_name'))]['add'] == 1)
          <!-- <a href="{{url('Employee/BareMetal/add')}}" class="btn btn-primary mt-3 m-3">Add</a> -->
          @endif
      </div>
    </div>
    ...
    @if(count($BareMetal) > 0)
        @foreach($BareMetal as $key => $BareMetals)
            @php
                $isEmployeeIdNull = is_null($BareMetals->employee_id);
                $isEmployeeIdMatch = !is_null($BareMetals->employee_id) && in_array(Auth::user()->id, explode(',', $BareMetals->employee_id));
            @endphp
                    ...
                    @if(in_array('BareMetal', array_column($RoleAccess, 'per_name')))
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end" style="">
                                @if($RoleAccess[array_search('BareMetal', array_column($RoleAccess, 'per_name'))]['update'] == 1)
                                    <li><a class="dropdown-item" href="{{url('Employee/BareMetal/edit/'.$BareMetals->id)}}">Edit</a></li>
                                @endif
                                @if($RoleAccess[array_search('BareMetal', array_column($RoleAccess, 'per_name'))]['delete'] == 1)
                                    <li><button class="dropdown-item delete_debtcase" url="{{url('Employee/BareMetal/delete/'.$BareMetals->id)}}" id="{{$BareMetals->id}}">Delete</button></li>
                                @endif
                            </ul>
                        </div>
                    @endif
...
<script>
    $(document).ready(function () {
        ...
        function fetchData() {
            ...
            $.ajax({
                url: "{{ url('Employee/BareMetal/get_BareMetal_yeardata') }}",
                method: 'GET',
