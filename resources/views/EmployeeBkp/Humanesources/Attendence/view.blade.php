        ...
        @if(count($attendance2) > 0)
            @foreach($attendance2 as $key => $row)
                @php
                    ...
                    $shifthours = is_numeric($row->shifthours) ? $row->shifthours : 0;
                    ...
                    // Convert total hours to decimal format and then to hours, minutes, and seconds
                    $totalHoursDecimal = $totalHours / 3600; // Convert seconds to hours
                    $totalHoursHours = floor($totalHoursDecimal); // Extract hours
                    $totalHoursMinutes = floor(($totalHoursDecimal - $totalHoursHours) * 60); // Extract minutes
                    $totalHoursSeconds = round(((($totalHoursDecimal - $totalHoursHours) * 60) - $totalHoursMinutes) * 60); // Extract seconds
                    $totalHoursFormatted = sprintf("%02d h %02d min %02d sec", $totalHoursHours, $totalHoursMinutes, $totalHoursSeconds);
                    
                    // Calculate break time and format it
                    $BreakTime = max(0, $totalHoursDecimal - $actualWorkingHours); // corrected this line
                    $BreakTimeHours = floor($BreakTime); // Extract hours
                    $BreakTimeMinutes = floor(($BreakTime - $BreakTimeHours) * 60); // Extract minutes
                    $BreakTimeSeconds = round((((($BreakTime - $BreakTimeHours) * 60) - $BreakTimeMinutes) * 60)); // Extract seconds
                    $BreakTimeFormatted = sprintf("%02d h %02d min %02d sec", $BreakTimeHours, $BreakTimeMinutes, $BreakTimeSeconds);
                    
                    // Calculate production hours and format it
                    $ProductionHours = max(0, $actualWorkingHours); // corrected this line
                    $ProductionHoursHours = floor($ProductionHours); // Extract hours
                    $ProductionHoursMinutes = floor(($ProductionHours - $ProductionHoursHours) * 60); // Extract minutes
                    $ProductionHoursSeconds = round((((($ProductionHours - $ProductionHoursHours) * 60) - $ProductionHoursMinutes) * 60)); // Extract seconds
                    $ProductionHoursFormatted = sprintf("%02d h %02d min %02d sec", $ProductionHoursHours, $ProductionHoursMinutes, $ProductionHoursSeconds);
                    ...
                    $totalTimeDecimal = $ProductionHours + $BreakTime + ($totalOvertimeSeconds / 3600); // Convert overtime to hours
                    $totalTimeHours = floor($totalTimeDecimal); // Extract hours
                    $totalTimeMinutes = floor(($totalTimeDecimal - $totalTimeHours) * 60); // Extract minutes
                    $totalTimeSeconds = round((((($totalTimeDecimal - $totalTimeHours) * 60) - $totalTimeMinutes) * 60)); // Extract seconds
                ...
                <tr>
                    ...
                    <td style="color:red">{{ $totalOvertimeFormatted }}</td>
                </tr>
            @endforeach
        @else
